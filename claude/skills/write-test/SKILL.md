...
2. Consider what the interface promises!
    ...
    - What is always/never true about the outputs of the unit under test?
...
1. Check what changed recently, either on the current branch, or since we branched off from the main branch!
...
- Integration with other components / services, so things like the web layer or the database layer, should be tested on the integration test level!
...
- Readable, focuses on the business process, not the technical necessities and boilerplate.
    - To achieve this, a BDD style SHOULD be followed, where the test method only contains high level concepts. For example:
```java
  @Test
...
- MUST test through APIs and interfaces whenever possible. Directly writing to and reading from databases is only acceptable when testing how something stores data in the database.
